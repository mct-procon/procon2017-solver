...
namespace PuzzleSolver.Core
{
    public class Solve
    {
        ...
        public Puzzle ConnectAuto(Puzzle puzzle)
        {
            ...
            Dictionary<ValueTuple<int, int>, T> scoreTable = new Dictionary<ValueTuple<int, int>, T>();
            
            for (int i = 0; i < polys.Count(); i++)
            {
                for (int j = 0; j < polys[i].Count; j++)
                {
                    scoreTable.Add(new ValueTuple<int, int>(i, j), new T(-1, 0, null, -1, -1, false));
                }
            }
                    ...
                    for (int dstPointId = 0; dstPointId < dstPoly.Count; dstPointId++)
                    {
                        for (int srcPointId = 0; srcPointId < srcPoly.Count; srcPointId++)
                        {
                            for (int option = 0; option < 4; option++)
                            {
                                ...
                                //スコアテーブルの更新
                                ValueTuple<int, int> Key = new ValueTuple<int, int>(dstPolyId, dstPointId);
            ...
            //結合度最大のペアを探す
            ValueTuple<int, int> bestKey = new ValueTuple<int, int>(0, 0);
            ...
            KeyValuePair<ValueTuple<int, int>, T> bestElement = new KeyValuePair<ValueTuple<int, int>, T>(bestKey, scoreTable[bestKey]);
